...
/*
 * Nhật ký update: mọi cập nhật của task (kể cả đổi trạng thái, đổi deadline, nhật ký gắn với task)
 * và nhật ký chung. Sheet "By time" xếp theo thời gian, "By task" gom theo task, "Summary" đếm theo người A → Z.
 * opts: { title, subtitle, rows: [{ date, time, task, topic, pic, status, statusKey, text, by, type }] }
 */
function buildUpdateLog(opts) {
  var rows = (opts.rows || []).slice();
  var header = ['Date', 'Time', 'Task', 'Update', 'Updated by', 'Topic', 'Person in charge', 'Current status', 'Type'];
  var widths = [12, 7, 48, 64, 16, 16, 16, 14, 16];
  function sheetRows(list) {
    var out = [header.map(function (h) { return { v: h, s: 'header' }; })];
    list.forEach(function (r) {
      out.push([rDateCell_(r.date), r.time || '', { v: r.task, s: 'wrap' }, { v: String(r.text || ''), s: 'wrap' }, r.by || '',
        r.topic || '', r.pic || '', r.statusKey ? { v: r.status, s: STATUS_STYLE_[r.statusKey] } : (r.status || ''), r.type || '']);
    });
    return out;
  }
  var byTime = rows.slice().sort(function (a, b) {
    return String(a.date).localeCompare(String(b.date)) || String(a.ts || '').localeCompare(String(b.ts || '')) || (a.seq || 0) - (b.seq || 0);
  });
  var byTask = rows.slice().sort(function (a, b) {
    return byName_(a.topic || '~', b.topic || '~') || byName_(a.task, b.task) || String(a.date).localeCompare(String(b.date)) ||
      String(a.ts || '').localeCompare(String(b.ts || '')) || (a.seq || 0) - (b.seq || 0);
  });
  var tRows = sheetRows(byTime);
  var last = Math.max(2, tRows.length);
  var people = [];
  rows.forEach(function (r) { if (r.by && people.indexOf(r.by) < 0) people.push(r.by); });
  people.sort(byName_);
  var types = [];
  rows.forEach(function (r) { if (r.type && types.indexOf(r.type) < 0) types.push(r.type); });
  var summary = [
    [{ v: opts.title || 'Update log', s: 'title' }],
    [{ v: (opts.subtitle ? opts.subtitle + ' · ' : '') + 'Exported ' + rNow_() + '.', s: 'muted' }],
    [],
    [{ v: 'Updated by', s: 'header' }, { v: 'Updates', s: 'header' }]
  ];
  people.forEach(function (n, i) {
    var r = 5 + i;
    summary.push([n, { f: "COUNTIF('By time'!$E$2:$E$" + last + ',A' + r + ')', v: rows.filter(function (x) { return x.by === n; }).length, s: 'number' }]);
  });
  var endPeople = 4 + people.length;
  summary.push([{ v: 'Total', s: 'total' }, { f: people.length ? 'SUM(B5:B' + endPeople + ')' : '0', v: rows.length, s: 'total' }]);
  summary.push([]);
  var typeHead = summary.length + 1;
  summary.push([{ v: 'Type', s: 'header' }, { v: 'Updates', s: 'header' }]);
  types.forEach(function (tp, i) {
    var r = typeHead + 1 + i;
    summary.push([tp, { f: "COUNTIF('By time'!$I$2:$I$" + last + ',A' + r + ')', v: rows.filter(function (x) { return x.type === tp; }).length, s: 'number' }]);
  });
  summary.push([{ v: '"By time" lists every update in the order it happened; "By task" groups the same updates by task.', s: 'muted' }]);
  var cols = widths.map(function (w) { return { width: w }; });
  var kRows = sheetRows(byTask);
  return buildWorkbook_([
    { name: 'Summary', columns: [{ width: 28 }, { width: 12 }], rows: summary },
    { name: 'By time', columns: cols, rows: tRows, freeze: 'A2', autoFilter: 'A1:I' + last },
    { name: 'By task', columns: cols, rows: kRows, freeze: 'A2', autoFilter: 'A1:I' + Math.max(2, kRows.length) }
  ]);
}
